...
# --platform opencode + --target /vault run launchme.sh non-interactively (skips the
# platform-select and vault-path prompts; the overwrite prompt only fires on an existing
# install, which a fresh image never has).
